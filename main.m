...
% *************************************************************************
%code_path    ="C:\Users\heywo\OneDrive - Queen's University\U of Toronto\Courses\Empirical Metric\Problem Set 1\Matlab\BLP - challenges";
...
for optrout=1:1
    ...
    perturbs=   (1:1:50)'; %dont care
    mytolx=         1e-3; %if within 1000 proximity considers equal
    mytolfun=       1e-3;
    mymaxiters=   5*10^5;
    mymaxfunevals=  4000;
   ...
   % fid = fopen(outfile,'w'); fclose(fid);
    ...
    % *********************************************************************
    % Demand instruments
    %Other cars characteritcs, correlated with price, but uncorrelated
    %hetergoenous of that one car(market share+price) (exclusive) 
    
    % what are some of the observable charactertics of other cars
    ...
    %main x variables are defined below
    X=[const,(hpwt),air,(mpg),space];
    
    %the following loop defines the BLP instruments-
    %%%% sum of the characteristics of all other competitors
    

    for i=1:size(id,1)    
    %id is the unique observation id
    % firmid is the firm id 
    %We will sum this value next to see how many such otehr models 
    ...
    %X = observables
    %sum_other,sum_rival-> instruments
    %covariance matrix
    ...
    %FIRST QUESTION IN THE ASSIGNMENT
    x1=[price,X]; clear X
    

    %variables in demand with random coefficients
    %SECOND QUESTION IN THE ASSIGNMENT
    x2=x1(:,1:size(x1,2)-1);
    ...
    for i=2:max(nmkt)
    nbrn(i)=sum(cdid==i);
    %sum number of id in the city 
    end

    %demographics and N(0,I) unobservables
    %we dont have demographics data 
    demogr=zeros(size(v));
    vfull = v(cdid,:);
    dfull = demogr(cdid,:);
    ...
    % *********************************************************************
    % Logit regressions (more interesting section)
    
    %outer loop: fix beta and volatility
    %inner loop: solve contraction mapping for each market, for each delta,
    %compute-> market share-> find a fixed pt base on contraction mapping and invert 
    %find the delta since it is invertible
    
    %delta = covariety *Beta + unobserved heterogenity 
    ...
    s_jt=share;
    %s_jt observed market share
    %outshr: the sigma that satisfis the contraction mapping
    y = log(s_jt) - log(outshr);
    %OLS: use y and use x1 that we define above(without random coefficient)
    ...
    %2SLS estimate for theta_1
    %below is the matrix for 2SLS
    %this is the regression of the delta on price and X
    mid = x1'*IV1*invA1*IV1';
    
    %2sls
    beta_iv = inv(mid*mid')*mid*y
    e=y-x1*beta_iv
    X_design = x1
    V_robust = inv(X_design'*X_design)*(X_design'*diag(e.^2)*X_design)*inv(X_design'*X_design)

    
    %theta1 are means of the random coefficient
    %theta2 are the sigma
    ...
    %now we get one share, you want to start looping
    % *********************************************************************
    ...
    for ppp=1:size(perturbs,1)
        ...
        %mvalold is the argument for exp(mvaold) in mkt share, this is the
        %numerator of how market share is calculated
        %mvaold_logit is the delta
        %sigma = the estimated market share
        ...
        %now going into the optimization
        % *****************************************************************
        % Quasi-Newton I
        % *****************************************************************
        if optrout==1
            options = optimset(...
                ...
                'MaxIter',     mymaxiters,...
                'Display','iter');
            %starting with theta 2, and do function minimzation on
            %constraint
            %using otpions using the gmm object
